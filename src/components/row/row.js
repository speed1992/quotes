...
export const Row = ({ data: { searchText, start, end, triggerChange, philosopherFullName, philosopherFullName_i10n, translateKey, audio, setAudio }, index, style }) => {
    ...
    if (!isUndefined(currentData[index]))
        return (
            <div onMouseMove={debouncedHandler} onTouchStart={debouncedHandler} key={index} className={evaluateClassNames(index)} style={style}>
                ...
                <Audio index={index} audio={audio} setAudio={setAudio} />
            </div >
        )
};